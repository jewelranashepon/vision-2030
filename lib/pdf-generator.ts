...
export class PDFGenerator {
  ...
  private addHeader(title: string, subtitle?: string) {
    ...
    this.doc.text(`Generated on: ${date}`, this.pageWidth - this.margin - 60, 25);
  ...
  private addSummaryCards(summary: ReportData['summary']) {
    ...
    const cards = [
      { label: 'Total Collection', value: `₹${summary.totalCollection.toLocaleString()}`, color: [34, 197, 94] },
      { label: 'Active Members', value: summary.activeMembers.toString(), color: [59, 130, 246] },
      { label: 'Average Monthly', value: `₹${summary.averageMonthlyCollection.toLocaleString()}`, color: [147, 51, 234] },
      { label: 'Highest Payment', value: `₹${summary.highestPayment.toLocaleString()}`, color: [249, 115, 22] },
    ];
  ...
  generateYearlyReport(data: ReportData, filters: any): void {
    ...
    const yearlyData = data.yearlyComparison.map(item => [
      item.year,
      `₹${item.amount.toLocaleString()}`,
      data.yearlyComparison.length > 1 ? this.calculateGrowth(item, data.yearlyComparison) : 'N/A'
    ]);
  ...
  generateMemberWiseReport(data: ReportData, filters: any): void {
    ...
    const memberData = data.memberWiseReport.slice(0, 10).map((member, index) => [
      (index + 1).toString(),
      member.membershipId,
      member.memberName,
    ...
    autoTable(this.doc, {
      head: [['Rank', 'Member ID', 'Name', 'Total Paid', 'Payments', 'Average', 'Last Payment']],
      body: memberData,
      startY: this.currentY,
      theme: 'grid',
  ...
  generateExecutiveSummaryReport(data: ReportData, filters: any): void {
    ...
    const kpiData = [
      ...
      ['Average Monthly Collection', `₹${data.summary.averageMonthlyCollection.toLocaleString()}`],
    ...
    const topMembers = data.memberWiseReport.slice(0, 5).map((member, index) => [
      (index + 1).toString(),
      member.memberName,
      member.membershipId,
      `₹${member.totalPaid.toLocaleString()}`
  ...
  generateDetailedTransactionsReport(data: ReportData, filters: any): void {
    ...
    const transactionData = data.monthlyCollection.slice(0, 50).map((item, index) => [
  ...
  private getFilterText(filters: any): string {
    ...
    if (filters.startMonth && filters.endMonth) {
      parts.push(`Period: ${this.formatMonth(filters.startMonth)} - ${this.formatMonth(filters.endMonth)}`);
    } else {
      if (filters.year && filters.year !== 'all') {
        parts.push(`Year: ${filters.year}`);
      }
      if (filters.month && filters.month !== 'all') {
        const monthName = new Date(2024, parseInt(filters.month) - 1, 1).toLocaleDateString('en-US', { month: 'long' });
  ...
  private calculateGrowth(current: any, yearlyData: any[]): string {
    const currentIndex = yearlyData.findIndex(item => item.year === current.year);
...
export async function generatePDFReport(
  ...
  const filterSuffix = filters.startMonth && filters.endMonth 
    ? `${filters.startMonth}-to-${filters.endMonth}`
    : `${filters.year !== 'all' ? filters.year : 'all-time'}-${filters.month !== 'all' ? filters.month : 'all-months'}`;
